...
defined('BASEPATH') OR exit('No direct script access allowed');
	...
	class Alquiler extends CI_Controller{	
		...
		public function deudores_reportes_management(){
				...
				$crud->columns('idContrato','idInmueble','edificio','locatario1','locador','Ultimo Pago','estado_contrato','contacto');
		...
		public function alquiler_management(){	
				...
				$crud->display_as('proxVenc','Próx.-Vto');
				$crud->display_as('ajuste','Ajuste en %');
				$crud->display_as('tipo_ajuste','Tipo de Ajuste');
				$crud->display_as('fecha_firma','Firma de Contrato');
				$crud->display_as('estado_contrato','Contrato');
				...
				$crud->required_fields('idInmueble','locador','operacion','locatario1','garante1','fechaInicio','punitorio','comision_admin','fechaPago','duracion','fechaFin','valor1','tipo_ajuste','ajuste');				
			...
			$crud->fields('idInmueble','locador','operacion','locatario1','locatario2','garante1','garante2','garante3','garante4','fechaInicio','duracion','fechaFin','fechaPago','proxVenc','punitorio','comision_admin','tipo_ajuste','ajuste','valor1','valor2','valor3','valor4','valor5','valor6','comision_inmo_a_pagar','sellado_contrato','fecha_creacion','usuario_creacion','fecha_firma','escribano','estado_alquiler','estado_contrato','pendientes');
				...
				if($sesion[0]==1 or $sesion[0]==3){
					$crud->add_action('Editar','','Alquiler/edit','ui-icon-pencil');
...
public function imprimir_gastos_alquiler(){
		...
		public function estado_contrato($value,$row){
			...
			$rescinde_fecha=$datos_rescinde[1];
			 ...
			 if($nro_pago<>""){$ultimo_pago=$this->buscar_datos_model->buscar_ultimo_pago($idC);}
	...
	public function edit(){
			...
			//aca se precarga el inmueble, el propietario y el valor			
					...
					$crud->callback_edit_field('punitorio', function ($value, $primary_key) {
						$punitorio = '<input id="field-punitorio" name="punitorio" type="text" value="'.$value.'" maxlength="3" style="width:40px;height:30px" class="numeric form-control" onclick="vaciar_input(this.id)" />';	
						return $punitorio;
					});
					...
					$crud->callback_edit_field('rescinde_dentro', function ($value, $primary_key){
						...
						$rescinde_dentro='<input id="field-rescinde_dentro" name="rescinde_dentro" type="numeric" min="0" max="6" value="'.$value.'" maxlength="1" style="width:50px;height:30px"  onkeyup="calcular_periodo_rescision()" onkeypress="return validateFloatKeyPress(this,event);"/>';
					...
					$crud->callback_edit_field('rescinde_fecha', function ($value, $primary_key){
						...
						$periodo='<input id="field-rescinde_fecha" name="rescinde_fecha" value="'.$value.'" style="width:80px;height:30px"/>';
		...
		public function fechaCreacion_Usuario_update($post_array, $pk){
			...
			//}elseif($rescinde_dentro==0){
				...
				//$post_array['rescinde_dentro']=0;
				//$post_array['rescinde_fecha']="";
				$post_array['estado_contrato']="VIGENTE";
			//}
